...
Planning now specifies the custom `v1.<payload-base64url>.<signature-base64url>` token, canonical payload order, HMAC-SHA256 signing, 30-minute expiry boundary, injectable clock/RNG ports, and token error codes approved by the user. No JWT dependency or additional business data is introduced.
