...
| BR-024 | A test requires at least four vocabulary items with four distinct meanings, uses four distinct meaning choices, randomizes questions and choices, and asks each eligible word once. Test drafts use a versioned HMAC-SHA256 signed snapshot with a 30-minute TTL; validity is `now < expiresAt`. |
